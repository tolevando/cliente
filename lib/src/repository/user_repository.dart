...
Future<void> setCreditCardPagarme(CreditCardPagarme creditCard) async {
  if (creditCard != null) {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setString('credit_card_pagerme', json.encode(creditCard.toMap()));
  }
}
...
Future<CreditCardPagarme> getCreditCardPagarme() async {
  CreditCardPagarme _creditCard = new CreditCardPagarme();
  SharedPreferences prefs = await SharedPreferences.getInstance();
  if (prefs.containsKey('credit_card_pagerme')) {
    print(await prefs.get('credit_card_pagerme'));
    _creditCard = CreditCardPagarme.fromJSON(json.decode(await prefs.get('credit_card_pagerme')));
  }
  return _creditCard;
}
