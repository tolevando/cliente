...
class CidadelaWidget extends StatefulWidget {
  @override
  _CidadelaWidgetState createState() => _CidadelaWidgetState();
}

class _CidadelaWidgetState extends StateMVC<CidadelaWidget> {
  SettingsController _con;
  
  _CidadelaWidgetState() : super(SettingsController()) {
    _con = controller;
    inicializaChecagem();
  }
  ...
  void inicializaChecagem() async{
    ...
    String cid = await _con.getCidadeSelecionada();
    if(cid!=null){
      Navigator.of(context).pushReplacementNamed('/Pages', arguments: 2);
    }else{

      if(settingsRepository.setting.value.cidades.length == 1){
        SharedPreferences prefs = await SharedPreferences.getInstance();
        Cidade _cidade = settingsRepository.setting.value.cidades.elementAt(0);
